...
int main(int argc, char **argv)
{
    ...
    while (!feof(stdin)) //? maybe still waiting for command
    {
        // get command line from input
        fputs(prompt, stdout); // * print ==>
        ...
        { 
            ...
            arg = args; // * arg -> args -> []
            *arg++ = strtok(linebuf, SEPARATORS); // *tokenize input; plus later
            ...
            { 
                ...
                { 
                    ...
                    strcat(cmndbuf, args[1]); //* "ls -al ."
                }
                else if (!strcmp(args[0], "environ"))
                { 
                    // "environ" command
                    char **envstr = environ; //! without &, environ is also **
                    while (*envstr)
                    { 
